...
def find_all_feature(train=pd.DataFrame(),test=pd.DataFrame()):
     ...
     for i in range(train.shape[0]):
         ...
         if i%100 == 0 and i!=0:
             print(i)
     with open('data/features/train_feature_article.txt','w') as f:
         for it in feature_train_article:
             f.write(it)
             f.write('\t')
     with open('data/features/train_feature_word_seg.txt','w') as f:
         for it in feature_train_word_seg:
             f.write(it)
             f.write('\t')
    ...
    for i in range(test.shape[0]):
        artcile = test.iloc[i,1].split(' ')
        word_seg = test.iloc[i,2].split(' ')
        feature_test_article = feature_test_article | set(artcile)
        feature_test_word_seg = feature_test_word_seg | set(word_seg)
        if i%100==0 and i!=0:
            print(i)
    with open('data/features/test_feature_article.txt','w') as f:
        for it in feature_test_article:
            f.write(it)
            f.write('\t')
    with open('data/features/test_feature_word_seg.txt','w') as f:
